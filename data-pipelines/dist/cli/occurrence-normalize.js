...
function aliasGet(rec, target) {
    ...
    for (const [k, v] of Object.entries(rec))
        normToValue[normalizeHeaderKey(k)] = v;
    for (const k of FIELD_ALIASES[target]) {
...
async function enumerateOccurrenceFiles(dir) {
...
async function loadStates() {
    try {
        return JSON.parse(await fs.readFile(PATHS.vendorStates, 'utf8'));
    }
    catch {
        return { type: 'FeatureCollection', features: [] };
    }
}
function joinState(lat, lon, statesFC) {
...
function stableId(source, occurrenceId, idx, file) {
    if (occurrenceId)
...
async function main() {
    ...
    const occDir = cfg.occurrenceDir ? path.resolve(cfg.occurrenceDir) : null;
    if (!occDir) {
        console.log('No occurrenceDir configured; skipping.');
        return;
    }
    const states = await loadStates();
    const files = await enumerateOccurrenceFiles(occDir);
    const metrics = { total: 0, validCoord: 0, inUS: 0, dateBuckets: {}, basis: {}, columnsSeen: {} };
    const outRows = [];
...
//# sourceMappingURL=occurrence-normalize.js.map