...
export function slugifySci(name) {
...
export function parseDateFields(rec) {
    const out = { confidence: 'none' };
    const y = toInt(rec.year ?? rec.eventYear);
    const m = toInt(rec.month ?? rec.eventMonth);
    const d = toInt(rec.day);
    const eventDate = (rec.eventDate ?? rec['event date'] ?? rec.date);
    if (eventDate) {
    ...
    const verb = (rec.verbatimEventDate ?? rec.occurrenceRemarks ?? '');
    if (verb) {
...
function monthFromStr(s) { const n = 'jan feb mar apr may jun jul aug sep oct nov dec'.split(' ').indexOf(s.slice(0, 3).toLowerCase()); return n >= 0 ? n + 1 : 1; }
...
export function isLikelyCaptive(rec) {
    const flags = [rec.isCaptive, rec.captive, rec.inCaptivity];
    if (flags.some((v) => String(v).toLowerCase() === 'true' || v === true || v === '1'))
        return true;
