...
async function loadOccurrenceData(filePath: string): Promise<SightingRecord[]> {
...
async function generateIndividualSightings(records: SightingRecord[]): Promise<void> {
  console.log('Generating individual sightings files...');
  
  // Group records by species
  const speciesGroups: Record<string, SightingRecord[]> = {};
  ...
  for (const [sci, speciesRecords] of Object.entries(speciesGroups)) {
    const features: SightingPoint[] = speciesRecords.map(record => ({
      type: 'Feature',
      geometry: {
        type: 'Point',
...
async function generateHeatMap(records: SightingRecord[]): Promise<void> {
  console.log('Generating heat map...');
  
  // Create points for all records
  const features = records.map(record => 
    turf.point([record.longitude, record.latitude], { 
      date: record.date,
      source: record.source,
...
async function generateSightingsIndex(records: SightingRecord[]): Promise<void> {
