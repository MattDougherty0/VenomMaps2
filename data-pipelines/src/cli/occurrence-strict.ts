...
type StrictRow = {
  ...
  h3_r6: string;
  h3_r5: string;
  tsMeta: 'high' | 'approx_month' | 'approx_year';
};

function hasMajorIssues(issues: any): boolean {
...
function passDate(rec: any): { ok: boolean; dateKey?: string; tsMeta?: 'high' | 'approx_month' | 'approx_year' } {
  const conf: string = rec.dateConfidence || 'none';
  const y: number = Number(rec.eventYear);
  const m: number | undefined = rec.eventMonth ? Number(rec.eventMonth) : undefined;
  const d: number | undefined = rec.eventDay ? Number(rec.eventDay) : undefined;
...
async function main(): Promise<void>{
  ...
  const dedup = new Map<string, { count: number; row: StrictRow }>();
  ...
  const audit: Record<string, number> = Object.create(null);
  const mark = (k: string) => { audit[k] = (audit[k]||0)+1; };

  for (const line of lines) {
    let rec: any;
    try { rec = JSON.parse(line); } catch { continue; }
    if (!rec) continue;
    if (!rec.inUS) { mark('drop_not_inUS'); continue; }
    const sci = slugifySci(String(rec.scientificName || ''));
    if (TARGET.size && !TARGET.has(sci)) { mark('drop_not_target'); continue; }
    if (!rec.insideExpertRange) { mark('drop_outside_range'); continue; }
    if (hasMajorIssues(rec.issues)) { mark('drop_issues'); continue; }
    const unc = rec.coordinateUncertaintyInMeters;
    if (Number.isFinite(unc) && Number(unc) > STRICT_CONFIG.maxUncertaintyMeters) { mark('drop_uncertainty'); continue; }
    if (rec.isCaptive) { mark('drop_captive'); continue; }
    let basisRaw = String(rec.basisOfRecord || '');
    if (!basisRaw) { basisRaw = 'Observation'; mark('missing_basis_kept'); }
    ...
    if (!allowedLower.has(basisNorm)) { mark('drop_bad_basis'); continue; }

    const datePass = passDate(rec);
    if (!datePass.ok || !datePass.dateKey) { mark('drop_no_date'); continue; }
    ...
    if (!dedup.has(key)) dedup.set(key, { count: 0, row: { sci, dateKey: datePass.dateKey, h3_r6: h3r6, h3_r5: h3r5, tsMeta: datePass.tsMeta! } });
    dedup.get(key)!.count++;
  }
  ...
  const perSpecies: Record<string, { lat: number; lon: number; ts: number; count?: number }[]> = {};
  ...
  const idx: { sci: string; count: number }[] = [];
