...
function gunzipMaybe(buf: Buffer, ext: string): Buffer {
  if (ext.endsWith('.gz')) {
...
export async function readRowsGeneric(file: string): Promise<Record<string, any>[]> {
  ...
  if (file.toLowerCase().endsWith('.csv') || file.toLowerCase().endsWith('.csv.gz') || file.toLowerCase().endsWith('.tsv') || file.toLowerCase().endsWith('.tsv.gz')) {
    ...
    return (res.data as any[]).filter(Boolean);
  }
  ...
  if (file.toLowerCase().endsWith('.geojson') || file.toLowerCase().endsWith('.geojson.gz')) {
    ...
    if (j && j.type === 'FeatureCollection') return j.features.map((f: any) => ({ ...(f.properties || {}), decimalLatitude: f.geometry?.coordinates?.[1], decimalLongitude: f.geometry?.coordinates?.[0] }));
    return [];
  }
  if (ext === '.xlsx') {
