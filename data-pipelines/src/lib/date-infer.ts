...
function toInt(v: unknown): number | undefined { const n = Number(v); return Number.isFinite(n) ? n : undefined; }
...
export type DateGuess = {
  ...
  month?: number;
  day?: number;
  confidence: 'high'|'text_inferred_full'|'text_inferred_month'|'text_inferred_year'|'none';
};

export function fromISOOrParts(record: Record<string, unknown>, iso?: string | null, y?: number, m?: number, d?: number): DateGuess | null {
  if (iso) {
  ...
  if (y && m && d) {
    ...
    const dt = new Date(s);
    if (!isNaN(dt.getTime())) return { date: s, year: y, month: m, day: d, confidence: 'high' };
  }
  return null;
}

export function fromExcelSerial(serial: number): DateGuess | null {
...
export function inferFromRecord(record: Record<string, unknown>): DateGuess {
  // Look for any date-like string or Excel serials
  for (const [k, v] of Object.entries(record)) {
    if (!DATE_LIKE_KEY.test(k)) continue;
    ...
    if (typeof v === 'number' && v > 24*3600*1000 && v < 1e13) {
      const dt = new Date(v);
      if (!isNaN(dt.getTime())) return { date: dt.toISOString().slice(0,10), year: dt.getUTCFullYear(), month: dt.getUTCMonth()+1, day: dt.getUTCDate(), confidence: 'text_inferred_full' };
    }
    ...
    if (mIso) {
      const yy = Number(mIso[1]); const mm = Number(mIso[2]); const dd = Number(mIso[3]);
      const candidate = `${yy.toString().padStart(4,'0')}-${mm.toString().padStart(2,'0')}-${dd.toString().padStart(2,'0')}`;
      const dt = new Date(candidate);
      if (!isNaN(dt.getTime())) return { date: candidate, year: yy, month: mm, day: dd, confidence: 'text_inferred_full' };
    }
    const mMon = s.match(MON_YEAR) || s.match(MONTH_YEAR);
    if (mMon) {
      const mm = monthFromName(mMon[1]); const yy = Number(mMon[2]);
      if (mm) return { year: yy, month: mm, confidence: 'text_inferred_month' };
    }
  }
  ...
  for (const v of Object.values(record)) {
    ...
    if (m) {
      const yy = Number(m[2]);
      if (yy >= 1800 && yy <= 2100) return { year: yy, confidence: 'text_inferred_year' };
    }
  }
